...
Test project: **client**. Run the full suite when changes span components, hooks, or services. Target a single file with `--testPathPattern` when only one component changed.

- If unsure which project/module a change touches, or changes span multiple projects/modules, run the full test suite with coverage: `cd client && npm test -- --watchAll=false`
...
Locate the coverage report your test command produced (Jest writes coverage to `client/coverage/lcov-report/` when run with `--coverage`).
...
See `.claude/rules/rules.md` for this project's test framework (Jest), assertion style (Jest expect / React Testing Library), and naming conventions. In general:
