...
4. Determine target files:
   ...
   - If no path, scan files changed in the current branch vs master: `git diff --name-only master...HEAD -- '**/*.{js,jsx,ts,tsx}'`
5. Filter to `src/**/*.{js,jsx,ts,tsx}`, exclude generated/build output directories (e.g. `obj/`, `bin/`, `dist/`, `node_modules/`) and any paths in the ignore list
