...
| **Automation** | Executable scripts (`.sh`, `.py`, `.js`), hook files, `.claude/settings.json`, or files with a shebang (`#!/`). Detected by extension, known hook paths, or shebang. | Skip `cd client && npm run build`/`cd client && npm test -- --watchAll=false`. Run syntax validation (`bash -n`, `python -m py_compile`). Review for correctness. |
...
3. For each selected item, implement the fix following these rules:
   ...
   - **Post-fix validation depends on scope type:**
     - **Source-only / Mixed (includes `src/` or `test/`)**: Run `cd client && npm run build` to verify the build still passes. If the build breaks, fix errors before proceeding.
