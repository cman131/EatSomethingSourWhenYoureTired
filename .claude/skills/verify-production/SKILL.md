...
Use console/local logs (no centralized platform) to query logs. Default time window for verification is `-2d`; narrow to `-4h` when a deploy time is known. Apply safe query practices: scope queries by service/component, start with summaries before raw output, and limit result volume.
...
Use console/local logs (no centralized platform) plus the scoping filter from Step 5.

Run in this order:

**Tier 1 — Did the entry point get hit?** (check server/access logs)
- For a new HTTP endpoint:
  ```
  grep "<new_route>" <access-log> | grep "<scoping_filter>" | tail -20
  ```

**Tier 2 — Were the new log statements emitted?** (check application logs)
For each distinctive log message template from Step 3:
```
grep "<distinctive substring>" <app-log> | grep "<scoping_filter>" | tail -20
```
If results are found, review the actual events.

**Tier 3 — Any errors/warnings from the new code?**
```
grep -i "error\|warning" <app-log> | grep "<new_class_or_module>" | grep "<scoping_filter>" | tail -20
```

**Tier 4 — Cross-service / external dependency check** (only if Step 3 identified adapter calls)
- Check any downstream service logs for errors correlated with the same time window.
...
|---|---|---|---|
| New endpoint hit | `grep "/v1/{resource}" <access-log>` | N calls in last 2d, 0 errors | ✓ exercised |
| New log emitted | `grep "foo bar" <app-log>` | N events, latest at {time} | ✓ emitted |
| Error rate from new code | `grep -i "error" <app-log> | grep "NewClass"` | N errors | ✓ clean / ⚠ investigate |
| Downstream call | Check downstream service logs | ... | ... |
