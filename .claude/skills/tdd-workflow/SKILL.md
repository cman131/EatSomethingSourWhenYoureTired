---
name: tdd-workflow
description: Use this skill when writing new features, fixing bugs, or refactoring code. Enforces test-driven development with 80%+ coverage using Jest, Jest mocks (built-in), and Jest expect / React Testing Library.
---
...
For each acceptance criterion, write test(s) that express the expected behavior. The exact syntax is Jest-specific (see the stack's `rules-addendum.md` for real syntax); the shape is the same everywhere:

```text
Jest test class: OrderServiceTests
  ...
  test: ProcessOrderAsync_WithValidInput_PersistsOrder
    ...
    Assert (Jest expect / React Testing Library) — result is not null; result.Status equals "Processed"
  ...
  test: ProcessOrderAsync_WithNullCustomerId_ThrowsArgumentException
    ...
    Assert (Jest expect / React Testing Library) — calling it throws ArgumentException
  ...
  parameterized test (inputs: "", " "): ProcessOrderAsync_WithEmptyOrderId_ThrowsArgumentException
    ...
    Jest's parameterized-test feature is for (e.g. Jest/Vitest's `it.each`,
...
```bash
cd client && npm test -- --watchAll=false --filter "FullyQualifiedName~ClassName"
...
Jest test: PersistWithDeduplicationAsync_WhenDuplicateReceived_SkipsPersist
  ...
  Assert (Jest mocks (built-in)) — repository.UpsertAsync was never called
```
...
- **Strict mock setups**: Use specific matchers for known values (Jest mocks (built-in)'s argument-matcher/predicate feature). A wildcard "any value" matcher is only acceptable for cancellation/timeout tokens and infrastructure/opaque parameters.
...
✅ CORRECT: One parameterized test covers all invalid inputs
  parameterized test (inputs: null, "", " "): ValidateAsync_WithInvalidInput_ReturnsFalse
    ...
    Assert (Jest expect / React Testing Library) — result is false
```
...
**Commands (side effects)** — use Jest mocks (built-in)'s verify/assert-called feature to confirm the right calls happened with the right arguments:

```text
Jest test: DeleteOrderAsync_PersistsDeletion
  Act — await sut.DeleteOrderAsync("order-1", "customer-1", cancellationToken)
  Assert (Jest mocks (built-in)) — repository.UpsertAsync was called exactly once with
...
❌ WRONG: Verifying the mock was called but never checking what was returned
  test: GetOrderAsync_WhenExists_Works
    ...
    Assert (Jest mocks (built-in)) — repository.GetByIdAsync was called with "order-1"
...
✅ CORRECT: Assert on the returned object
  test: GetOrderAsync_WhenExists_ReturnsMappedOrder
    ...
    Assert (Jest expect / React Testing Library) — result is not null; result.Id == "order-1"; result.ProductId == "prod-42"
```

**Mixed (command + return)** — some methods do both. Verify the side effect AND assert the return value:

```text
Jest test: SyncOrderAsync_PersistsAndReturnsUpdatedOrder
  ...
  Assert (Jest expect / React Testing Library) — result.IsActive is true
  Assert (Jest mocks (built-in)) — repository.UpsertAsync was called exactly once with an order where IsActive == true
```
...
✅ CORRECT: Calls the real method, asserts real output
  test: GetProcessedOrdersAsync_ReturnsOnlyProcessedOrders
    ...
    // Assert
    Assert (Jest expect / React Testing Library) — result has 2 items; every item has Status == Processed
```

## Testing Patterns

### Service Test Pattern (using Jest mocks (built-in) and Jest expect / React Testing Library)
```text
class OrderServiceTests
  ...
  test: GetOrderAsync_WhenExists_ReturnsOrder
    ...
    result = await sut.GetOrderAsync("order-1", cancellationToken)
    Assert (Jest expect / React Testing Library) — result is equivalent to expected
  ...
  test: GetOrderAsync_WhenNotFound_ReturnsNull
    ...
    Assert (Jest expect / React Testing Library) — result is null
```
...
class OrderRepositoryTests
  ...
  test: UpsertAsync_WithValidOrder_PersistsDocument
    ...
    Assert (Jest mocks (built-in)) — dataStoreConnection.UpsertItemAsync was called exactly once with an order where Id == "order-1"
  ...
  test: UpsertAsync_WithConcurrencyConflict_ThrowsConcurrencyException
    ...
    Assert (Jest expect / React Testing Library) — calling it throws ConcurrencyException
```

### Idempotency Test Pattern
```text
Jest test: ProcessOrderAsync_WhenCalledTwice_ProducesSameResult
  ...
  Assert (Jest mocks (built-in)) — repository.UpsertAsync was called exactly twice
```
