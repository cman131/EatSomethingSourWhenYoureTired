...
description: Investigate a GitHub Issues bug end-to-end — read the bug report, trace relevant code paths, query production logs, and synthesize a root cause analysis.
---

# Bug Troubleshooter (Logs + Code)

Investigate a GitHub Issues bug by reading the bug report, tracing relevant code paths, querying production logs, and synthesizing a root cause analysis.

## Input

The user provides a GitHub Issues work item / issue ID via `$ARGUMENTS`. Examples:
- `3922115`
...
Default time window for bug investigation is `-2d`. Use console/local logs (no centralized platform) to query logs — check application output, stdout/stderr, and local log files. Apply safe query practices: scope queries by service/component, start with summaries before raw output, and limit result volume.
...
1. Fetch the work item/issue using GitHub Issues's "get work item" tool, scoped to `mahjong-site`, with full detail/expansion.
2. Fetch comments using GitHub Issues's "list work item comments" tool, scoped to `mahjong-site`.
...
1. **Identify the dependency**: Map the interface name to either a log-observable service or an unobservable managed/third-party dependency.
2. **Log-observable services**: Check their local log output for errors or warnings correlated with the incident time window.
3. **Unobservable managed/third-party dependencies**:
   - Errors from these will appear as exceptions in this service's own logs. Search for exception stack traces around the incident time.
   - For dependencies whose own logs live outside your local environment, direct the user to that dependency's own console/logs for root cause.
...
|---|---|---|---|
| 1 | `grep "Error" <log-file>` | -2d | Found N occurrences of ... |
