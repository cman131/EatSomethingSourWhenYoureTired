...
Examples:
- `mahjong-site/plans/home-page-health-score-addin/prd.md 3701585`
...
Call your issue tracker's work-item-lookup tool (`GitHub Issues`) with `project: "mahjong-site"` and the Epic ID. Display its title so the user can confirm it is the right parent. If the work item does not exist or its `workItemType` is not `Epic`, stop and tell the user.
...
> **Feature created: [#\<id\> — \<title\>](https://dev.azure.com/{your-org}/mahjong-site/_workitems/edit/\<id\>)**
