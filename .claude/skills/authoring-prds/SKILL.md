---
name: authoring-prds
description: Use when creating a new PRD from a feature idea — acting as a Scrum Product Owner to interview a stakeholder, fill gaps, and resolve open questions before writing. For plans in the mahjong-site repo.
---
...
Read `README.md`, `docs/product-vision.md (not yet written)`, and the most recent existing PRD (style reference). Grep sibling repos for prior art on technical dependencies named in the brief. Then brief the user: *"I found X already exists in Y — does it apply here?"* Also check whether the brief's proposed solution is the right one — a brief saying "add an email feature" might be solving a notification-volume problem that has a better solution. Also scan the other PRDs under `plans/`: does this feature depend on another initiative shipping first (a **prerequisite**), or share a surface or data source with one? Note candidate dependencies and confirm them in the interview, same as other prior art.
...
Preserve resolved decisions in §9.1 Resolved Decisions — never delete the audit trail.
...
**6. Self-review.**
Check for: implementation detail that crept in, ACs that depend on unresolved spikes (add *"degrades to … if unresolved"* notes), invented metric targets (ground them or mark *"TBD — pending baseline data"*), hardcoded assumptions that should be open questions. Also **check the draft against `docs/product-vision.md (not yet written)`'s PRD alignment questions** — this is a required gate, not a mental note. Then run two final checks:
- **Dependency check:** every prerequisite or dependent initiative is named explicitly in §6 External Dependencies (marked **Prerequisite**) and the appendix.
- **Open-question audit:** re-read each §9.2 item against sibling PRDs, the product vision, design artifacts, and local repos from Step 1.5. Demote any that are now answerable into §9.1 Resolved Decisions.
...
Strip anything that describes *how* at a service/API level. Describe **observable behavior** — what the user sees, does, and can rely on — never the mechanism behind it. This applies just as much when **revising** a PRD to reconcile it with implementation decisions (ADRs, new services, an async rewrite) as it does at first authoring. Reconciliation is the *most common* source of engineering-detail drift: someone updates a user-facing section to match how the code now works and pastes in the code's vocabulary. Keep the behavior; drop the vocabulary.

<!-- EXAMPLE: replace with a worked example from your own product domain -->

**Async / eventually-consistent flows are still in scope for the PRD — as UX.** When a result isn't instant, capture what the user observes: a pending state, an eventual confirmation, a failure they can retry. That is a legitimate product requirement, not implementation detail. What to leave out is the *mechanism* — orchestration engines, workers, queues, polling, internal state/enum names, DB or field names, HTTP verbs and endpoint paths. Those live in `implementation-plan.md` and the technical-reference section, and the PRD can point to them.
...
| "No need to mention the other PRD in §6" | If the feature can't ship until another initiative does, that's a prerequisite — name it in §6 External Dependencies (marked **Prerequisite**) and the appendix. |
...
- Wrote §6 External Dependencies without asking whether another initiative must ship first.
- Reconciling a section to the code's current state — and pasting in internal state/enum names, API endpoints, or field paths instead of the behavior the user observes. Keep the behavior; move the mechanism to the implementation plan (see PO Altitude Rule).
