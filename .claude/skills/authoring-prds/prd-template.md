...
**Product Area:** Mahjong Club Platform
**Parent PRD:** N/A
...
<!-- Prose description of the feature, organized by surface or user flow.
     ...
     It's fine to name which existing services are involved, but do not describe API contracts,
     payload shapes, or internal mechanics.
...
- **Who can access this feature?** JWT-based auth (registered users only) — describe the roles/entitlements/flags that gate visibility and action.
- **What's the fallback for users without access?** Describe the no-access experience (hidden entirely vs. visible-but-disabled vs. upsell prompt).
...
<!-- Group ACs by functional area (matching §4 sub-sections is a good default).
     ...
     Describe observable behavior, not the mechanism — including for async results: "the row shows an
     Applying… state, then a confirmed state" not internal status enums, workers, polling, or field paths.
     Include a Permissions & Entitlements AC group that mirrors §5 (access model, fallback for users without access).
...
<!-- §9.1 preserves decisions already reached — do not delete these; they are the audit trail.
     §9.2 lists only what is genuinely unanswerable today.
     Every open item needs an Owner (Engineering or Product) and a Priority (High/Medium/Low).
     High-priority open items that block ACs should have a contingency noted in §7. -->
...
- Product Vision: `docs/product-vision.md (not yet written)`
